    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(userAddress) in userAddressList" :key="userAddress.id">
        <span class="username" :class="{selected: userAddress.isDefault==='1'}"
         @click="changeSelected(userAddress,userAddressList)">{{userAddress.consignee}}</span>
      ...
      <div class="detail">
        ...
        <ul class="list clearFix" v-for="(goods) in detailArrayList" :key="goods.skuId">
          ...
          <li>
            <p style="width:635px;">{{goods.skuName}}</p>
            <h4>7天无理由退货</h4>
          </li>
          <li>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //选择地址
      changeSelected(userAddress,userAddressList){
        //遍历，先将所有的变回初始值
